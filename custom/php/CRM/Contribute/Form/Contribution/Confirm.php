...
class CRM_Contribute_Form_Contribution_Confirm extends CRM_Contribute_Form_ContributionBase {
  ...
  static function processContribution(&$form,
    ...
    if (!$online || $form->_values['is_monetary']) {
      if (!CRM_Utils_Array::value('is_pay_later', $params)) {
        if(empty($params['payment_method']))
            ...
            $contribParams['payment_instrument_id'] = $params['payment_method'];
      }
    }
    ...
    if ($contribution && $contribution->id && $form->_priceSetId) {
      ...
      if ( (empty($form->_lineItem) || !$form->_separateMembershipPayment) && CRM_Utils_Array::value('is_quick_config', $form->_params)) {
        $form->_lineItem = null;
      }
    }
  ...
  function processRecurringContribution(&$form, &$params, $contactID, $contributionType, $online = TRUE) {
    ...
    if (!$online || $form->_values['is_monetary']) {
        $ins = CRM_Utils_Array::value('payment_method', $params);
        if(empty($ins))
            $recurParams['payment_instrument_id'] = 1;
        else
            $recurParams['payment_instrument_id'] = $ins;
    }
